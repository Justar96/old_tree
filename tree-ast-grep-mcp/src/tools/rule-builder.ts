...
export class RunRuleTool {
  ...
  private buildYaml(p: RuleBuilderParams): string {
    ...
    if (p.insidePattern) lines.push(`    inside: { pattern: ${JSON.stringify(p.insidePattern)} }`);
    if (p.hasPattern) lines.push(`    has: { pattern: ${JSON.stringify(p.hasPattern)} }`);
    if (p.notPattern) lines.push(`    not: { pattern: ${JSON.stringify(p.notPattern)} }`);
    if (p.where && p.where.length > 0) {
      lines.push('    where:');
      for (const w of p.where) {
        const row: string[] = [];
        row.push(`metavariable: ${w.metavariable}`);
        if (w.regex) row.push(`regex: ${JSON.stringify(w.regex)}`);
        if (w.notRegex) row.push(`not: { regex: ${JSON.stringify(w.notRegex)} }`);
        if (w.equals) row.push(`equals: ${JSON.stringify(w.equals)}`);
        if (w.includes) row.push(`includes: ${JSON.stringify(w.includes)}`);
        lines.push(`      - ${row.join(', ')}`);
      }
    }
    if (p.fix) lines.push(`    fix: ${JSON.stringify(p.fix)}`);
    return lines.join('\n');
  }
}
