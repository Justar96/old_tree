...
export class ScanTool {
    ...
    async execute(params) {
        ...
        try {
            ...
            if (sanitizedParams.ruleIds && sanitizedParams.ruleIds.length > 0) {
                findings = findings.filter(finding => sanitizedParams.ruleIds.includes(finding.ruleId));
            }
            return {
                findings,
                summary: {
    ...
    async scanWithoutRules(params, resolvedPaths) {
    ...
    async createTemporaryRulesFile(rulesContent) {
